...
**Future detection:** this was a one-off verification. The standing check that
consumes `pluck-diagnostics.json` and files a task bead only on reproduced
starvation is [scripts/starvation-watch.py](../../scripts/starvation-watch.py) —
see [starvation-watch.md](starvation-watch.md) for the design and operations.
