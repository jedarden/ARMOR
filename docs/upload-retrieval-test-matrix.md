...
| U6 | **Out-of-order / parallel part upload** | **boto3/SDK default: parts upload concurrently** | L3 `TestMultipartSuspectPatterns/U6_*` (added 2026-07-15) | ✅ **rejected with 400**: ARMOR requires sequential part upload; parallel uploads must be disabled in SDK config |
| U7 | **Part retry** (same partNumber re-uploaded after network failure) | all SDKs retry parts | L3 `TestMultipartSuspectPatterns/U7_part_retry` (added 2026-07-15) | ✅ **rejected with 400**: part retries rejected; entire upload must be retried from beginning |
| U8 | **Non-block-aligned intermediate parts** (e.g. 10,000,000-byte parts; anything not a multiple of 64KiB) | configurable in every SDK | L3 `TestMultipartSuspectPatterns/U8_non_block_aligned_part` (added 2026-07-15) | ✅ **rejected with 400**: parts must be multiples of BlockSize (65536 bytes); clients must use aligned part sizes (e.g. 5,242,880 for 5MiB, 16,777,216 for 16MiB) |
...
3. ~~**`bf-59unr3` (P1): U6/U7/U8 are design-level suspects, not regression risks.**~~ **✅ FIXED 2026-07-15**: ARMOR now rejects out-of-order parts, part retries, and non-block-aligned parts with explicit 400 errors. Clients must configure SDKs for sequential, block-aligned, no-retry behavior (disable parallel uploads, use part sizes that are multiples of 64KiB, and retry entire uploads on failure).
