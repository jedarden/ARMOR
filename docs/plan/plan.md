...
stringData:
  ...
  master-encryption-key: "REMOVED-NOT-A-SECRET-VALUE"
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
curl -s -H "Authorization: Bearer REMOVED-NOT-A-SECRET-VALUE \
...
- [x] **Restorability metrics and alerting** — **done** (bf-1pphhz): the verifier publishes per-bucket gauges `armor_last_verified_restore_timestamp` / `armor_verified_object_ratio` / `armor_restore_verification_failures_total` (hooks in `internal/restoreverifier`), and `declarative-config` carries the Grafana dashboard (`grafana-dashboard-restore-verifier`) plus a per-bucket `PrometheusRule` (`ArmorRestoreVerificationStale` >12h, `ArmorRestoreVerificationFailures` >0 in 1h). **Caveat:** the six ARMOR clusters run VictoriaLogs, not Prometheus, so the `ServiceMonitor`/`PrometheusRule` are inert until a Prometheus scrapes `/metrics` — the Deployment still proves restorability by logging every cycle regardless. **Activation (2026-09-25, armor-afe279bd):** iad-ci now scrapes both surfaces and evaluates the shipped rules — VictoriaMetrics (already present for capacity work) gained an `armor` scrape job (server `:9001` admin mux + restore-verifier `:9002`), and `vmalert` evaluates the contract-pinned rule group verbatim, notifying Alertmanager → the existing ntfy channel. The `.disabled` manifests stay `.disabled` there (no prometheus-operator CRDs); other clusters remain inert. Verified by `scripts/alerting-smoke-test.sh` (live collection/evaluation/consistency/delivery) and `scripts/alerting-rule-unittest.sh` (offline freshness-failure and multipart-transition firing).
- [x] **Failure escalation** — **done** (`internal/restoreverifier/escalation.go`, wired via `verifyBucket`/`escalateResult` in `verifier.go`): one bead per distinct active failure (dedupe key = bucket + object key + path + failure class) carrying object key, bucket, deployment, envelope version from provenance, and both-path evidence; staleness escalates one bead per freshness window per bucket. A persisted dedupe set makes it storm-proof — a failing object never files more than one bead, and a failed filing records nothing and never loops (bounded by scheduler cadence), so it cannot recreate the 2026-07 unattended retry-storm pattern. **Deployment enablement (2026-09-25, armor-babc0b2b):** filing goes through the canonical bead-rs CLI (`BeadRSFiler`: `bead create --issue-type bug --unique-ref restore-verifier:<sha256 of dedupe key>`, EXISTING/EXISTING_CLOSED answers recorded rather than re-filed — store-level idempotence covers a lost state file; staleness refs anchor to the window index). The restore-verifier image ships the pinned `bead` release binary (runtime stage `scratch` → `debian:bookworm-slim`: the release binary is glibc-dynamic), startup validation (`BeadRSFiler.ValidateStartup`: CLI runnable, volume writable, `bead init` provisioning, `bead list` open) logs `ESCALATION FILING DISABLED —` + remediation instead of degrading verification, and `iad-ci/armor` is enabled live (PVC `sata` at `/var/lib/restore-verifier`, `VERIFIER_ESCALATION=true`); the other three deployments stay off pending the same volume + env.
- [x] **Scheduled DR drill** — **automation built** (`ModeDRDrill`/`runDRDrill` in `internal/restoreverifier`, on-demand `POST /trigger?mode=dr-drill`, and a `-dr-drill-interval` / `VERIFIER_DR_DRILL_INTERVAL` scheduler): runs the `armor decrypt`-only drill (MEK unwrap → raw B2 fetch → ADR-003-aware decrypt → checksum → application assertion) with the ARMOR read path deliberately excluded, proving the "ARMOR server is gone" recovery path from `docs/disaster-recovery.md`. **Production cadence enabled (2026-09-25, armor-445bcb28):** all four restore-verifier Deployments set `VERIFIER_DR_DRILL_INTERVAL: "24h"` (declarative-config `1550e3e8`, 2026-08-28 — the original caveat that the cadence was unset everywhere and the drill was on-demand-only held from 2026-07-19 until then), and live pods confirm scheduled drills execute and report results (iad-ci 2026-09-24 and rs-manager 2026-09-25 each logged a full drill with every sampled object recovered direct-only). Scheduling semantics, result reporting, and the safe-pause lever (set the interval to `0` in declarative-config; on-demand trigger stays available) are documented in the [restore-verifier deployment guide](../restore-verifier-deployment-guide.md); the scheduler-level contract is pinned by `TestStartScheduledDrillExecutesAndReports` / `TestStartWithoutDrillIntervalLeavesDrillPaused` / `TestStartScheduledDrillFailureSignalsWithoutRetryStorm` (damaged ciphertext must fail subsequent drills into the documented health signals — stale `drill_last_success`, advancing failure counter, zero ratio — with no bead filed, no ARMOR-read fallback, and a run rate still bounded by one per tick; armor-a166a6bc) in `internal/restoreverifier/drill_schedule_test.go`.
...
- After V2 is deployed and migration has run on a bucket, the bucket's MEK is
  ...
  disclosed. Rotation goes through the **MEK key ring (§8.13)**, which removes
  the irreversible failure mode of today's rotation (a MEK mismatch between
  OpenBao and the rotate call, or a restart mid-run, makes rotated objects
  unreadable); with the ring, per-bucket rotation is a bead blocked on the
  ring release and that bucket's migration. **Retiring** an old key from the
  ring (a key deletion) stays an operator action.
...
#### 8.13 MEK key ring — multiple concurrent MEKs (P1)

Findings (2026-08-28): `internal/keymanager` holds exactly one MEK per
key-id with no fallback, so rotation's safety comes entirely from ordering
(OpenBao → ESO → `POST /admin/key/rotate` → restart, byte-identical values,
never restart mid-run). The old MEK exists only in the memory of the replica
that served the POST; on ord-devimprint (`replicas: 3`) the other two
replicas cannot unwrap already-rewrapped objects until the rollout finishes;
`CopyObject` exceptions (> 5 GiB) stay on the old MEK indefinitely; and a
mismatch or a mid-run restart loses data because `REPLACE` overwrote the old
wrapped DEK and the bucket has no versioning. The model to copy is Sealed
Secrets: keep every historical key, encrypt with the newest, decrypt with
whichever key matches, re-encrypt separately and optionally.

Decisions:
- **Ring configuration.** `ARMOR_MEK` stays the *active* key (wraps every new
  DEK). `ARMOR_MEK_RING` is a comma-separated list of retired-but-valid hex
  MEKs; `ARMOR_MEK_<NAME>_RING` likewise per named key. Every replica loads
  the whole ring, so there is no mixed-replica window and a restart mid-
  rotation is harmless. Keys are **generated by a human or agent into
  OpenBao** (`openssl rand -hex 32 | bao kv put … =-`) — never by the server
  (a self-minted key is an un-escrowed key); only the *use* of many keys is
  automatic.
- **Key identification.** A MEK's fingerprint is the first 8 bytes of
  `SHA-256(MEK)` as 16 hex chars (what `.armor/rotation-state.json` already
  records). There is no free B2 metadata slot (a multipart named-key object
  uses all 10 headers), so the fingerprint is encoded **inside the existing
  value**: `x-amz-meta-armor-wrapped-dek: v2:<fp16>:<base64>`. Unwrap parses
  the prefix and selects that key; a legacy value without the prefix is
  trial-unwrapped across active + ring (AES-KWP's integrity check makes a
  wrong key fail cleanly). The manifest entry and the metadata cache carry the
  fingerprint alongside the wrapped DEK. `armor decrypt` and
  the restore-verifier accept a ring (flag list or the escrow file's `mek_ring`
  array) and select by fingerprint the same way.
- **Rotation.** `POST /admin/key/rotate` no longer takes the old key from
  process memory: it re-wraps every object whose fingerprint ≠ the active key's
  fingerprint to the active key, resumable via the existing state file; the
  request body carries nothing secret (the active key is already loaded from
  config). `GET /admin/key/ring` returns, per key-id, the active fingerprint,
  the ring's fingerprints, and an object-count histogram by fingerprint from
  the manifest. **Retiring** a key = removing it from `ARMOR_MEK_RING` in
  OpenBao, permitted only when its histogram count is 0 (`armor check` warns
  otherwise) — a key deletion, therefore operator-only.
- **Procedure** (replaces the four-step runbook): add the new key to OpenBao
  and make it active with the old one appended to the ring → rolling restart
  → `POST /admin/key/rotate` (optional, background) → retire when the
  histogram is zero. `CopyObject` exceptions simply stay on their ring key
  until the format migration re-uploads them. Escrow is of the ring as a unit.
