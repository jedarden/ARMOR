...
- `UploadPart` derives each part's CTR start block from the **cumulative sizes of all lower-numbered parts**; since that requires knowing every lower part's size first, **parts must be uploaded sequentially** — out-of-order/concurrent parts are rejected with `InvalidPartOrder`. Standard concurrent clients (aws cli defaults, litestream) must be configured for serial part upload (decision on true out-of-order support: bf-59unr3).
