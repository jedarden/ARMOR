...
- **Out-of-order and concurrent part uploads are supported via a uniform-part-size contract** (see [ADR-005](../adr/005-out-of-order-multipart-uniform-part-size.md)): part size `P` is pinned by the first arriving part, part `N`'s CTR start block is `(N−1) × P / blockSize` — a function of the part *number*, not arrival order — and the final part is the only one allowed to be smaller than `P` (uniformity is validated at CompleteMultipartUpload; violations hard-fail, never store). Retried parts are idempotent. This is what lets unmodified concurrent clients (aws cli defaults, SDK uploaders, litestream) work. *Implementation status: decided 2026-07-19; until it ships, the interim behavior is sequential-only enforcement (`InvalidPartOrder` on out-of-order arrival).*
