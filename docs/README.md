...
- **[ARMOR_PREFIX Cutover and Legacy Internal-State](runbooks/prefix-cutover-and-legacy-state.md)** — Arming `ARMOR_PREFIX` on a deployment that already writes at the bucket root: what moves and what stays, collision avoidance, the data move with per-state treatment (manifests, HMAC sidecars, ADR-016/ADR-003 sidecars, multipart state, provenance), validation across both eras, and a rollback that is a data move, not an env flip
- **[Restore-Verifier Alerting Runbook](runbooks/restore-verifier-alerting.md)** — The live alerting pipeline activated on iad-ci (VictoriaMetrics scrape → vmalert evaluation → Alertmanager → ntfy): the tailnet-only surfaces, the two verification scripts and when to run them, the three-verbatim-copies rule for changing a shipped alert, iad-ci ingest facts that have already cost time (the verifier's never-run canary family dropped at scrape, string-valued gauges never stored, `.disabled` manifests staying disabled), page triage, and what activating another cluster takes
