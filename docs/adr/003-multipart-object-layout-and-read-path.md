...
5. **Hard-fail part validation:** any part pattern ARMOR cannot encrypt correctly is rejected at request time rather than stored corrupted. The principle is current — ADR-015 inherits it verbatim — but the specific rejections below are the 2026-07-18 set; each is annotated with what governs today:
   ...
   - Intermediate (non-final) part whose size is not a multiple of the block size → `InvalidPartSize` (400), with a message telling the client to use a block-aligned part size (e.g. 5 MiB, 16 MiB). *(Superseded by ADR-011: alignment binds only a part that another part is placed after, on a uniform v2 upload — part 1, the presumed-final part, and every ADR-011 non-uniform upload are exempt. Format v3 has no alignment rule at all.)*
   - Completion referencing unknown or inconsistent parts → `InvalidPart` / `InvalidPartOrder`. *(Current form: an unknown part, or a completion whose part 1 never arrived, is still `InvalidPart`; uniformity, alignment, and the 5 MiB non-final minimum — enforced at Complete against part 1's size on both formats — are `InvalidPartSize`; `InvalidPartOrder` is gone.)*
