...
//    - Content lines (indented, no colon): MappingKey or Unknown
//    - Lines starting with !: Tag (syntactically correct YAML)
//    - Lines with ! in middle/end: MappingKey or Unknown
//    - See Section 12B.2 test_folded_scalar_continuation_lines_with_exclamation_marks()
//
// 6. Exclamation Mark Patterns (Section 12B):
//    - Keys ending with !: "key!:" → should be MappingKey (not Tag)
//    - Keys with ! in middle: "key!value:" → should be MappingKey
//    - Multiple !: "key!!value:" → should be MappingKey
//    - Continuation lines with !: "  text! here" → MappingKey or Unknown
//    - Continuation starting with !: "  !important" → Tag
//    - See test_folded_block_scalar_with_exclamation_marks() for examples
//
// Example Usage:
// -------------
// ```rust
...
// ```
//
// Helper Functions for Dynamic Test Generation:
// --------------------------------------------
// - create_folded_scalar_test(indent, key, modifier, indent_num) → creates single tuple
// - generate_folded_scalar_tests_multi_level(keys, modifiers, indent_levels) → bulk generation
// - generate_folded_scalar_tests_all_levels(keys, modifiers, indent_nums) → comprehensive
// - generate_folded_scalar_tests_for_level(level_name, keys, modifiers, indent_nums) → level-specific
// - See Section 12B.3 for detailed helper function documentation
//
// GAPS IN EXPLICIT INDENT COVERAGE
// --------------------------------
// As of 2026-07-13 (Section 12B analysis), the following explicit indent test patterns are documented but not fully implemented:
//
...
//   - (Repeat for 6-space, 8-space, tab)
...
//   * Pattern: vec![(line, vec![LineType::MappingKey, LineType::Unknown])]
...
//   * Multiple consecutive !: "key!!!:" → MappingKey
...
//   * Pattern 2: Tab indentation with "\t" prefix
...
//     "Folded scalar continuation line with ! should be MappingKey or Unknown: '{}' (got {:?})",
...
//     continuation_result,
//     LineType::Tag,
//     "Continuation starting with ! is classified as Tag: '{}'",
//     continuation_line
// );
// ```
//
...
// - Edge case - single character keys: ("  a!: >-1", "a!", LineType::MappingKey)
// - Edge case - keys ending with !: ("  end!with!bang!: >-1", "end!with!bang!", LineType::MappingKey)
...
// - Pattern 5.D: vec![LineType::Tag, LineType::MappingKey, LineType::Unknown] for lines starting with !
...
#[test]
fn test_folded_scalar_strip_explicit_indent_modifiers_at_2_space() {
    // Test folded scalars with strip explicit indent modifiers: >-n for n=1-9
    // At 2-space indentation level only
    // This provides focused coverage of strip explicit indent (>-n) specification for folded scalars
    // Follows the pattern established in test_folded_scalar_basic_modifiers_at_various_indentation_levels

    let test_cases = vec![
        // ===== Level 1: 2-space indentation with strip explicit indent >-n =====
        // Strip >-n (n=1-9) - main test cases
        ("  text1: >-1", "text1", LineType::MappingKey),
        ("  text2: >-2", "text2", LineType::MappingKey),
        ("  text3: >-3", "text3", LineType::MappingKey),
        ("  text4: >-4", "text4", LineType::MappingKey),
    ...
    for (line, expected_key, expected_type) in test_cases {
        ...
        assert_eq!(
            result, expected_type,
            "Folded scalar strip explicit indent test failed: '{}' - expected {:?}, got {:?}",
            line, expected_type, result
        );
        ...
        if result == LineType::MappingKey {
            ...
            assert!(
                info.is_some(),
                "Should detect mapping key for folded scalar with strip explicit indent: '{}'",
                line
            );
            let detected = info.unwrap();
            assert_eq!(
                detected.key, expected_key,
                "Key mismatch for folded scalar with strip explicit indent: '{}' - expected '{}', got '{}'",
                line, expected_key, detected.key
            );
        }
    }

    // Test continuation lines for folded scalars with strip explicit indent modifiers
    let continuation_lines = vec![
        // Level 1 continuation lines with ! characters
        ("  This is content with! exclamation", vec![LineType::MappingKey, LineType::Unknown]),
        ("  More! text! here! for! testing!", vec![LineType::MappingKey, LineType::Unknown]),
    ...
    for (line, expected_types) in continuation_lines {
        ...
        assert!(
            expected_types.contains(&result),
            "Continuation line for folded scalar strip explicit indent should be one of {:?}: '{}' (got {:?})",
            expected_types, line, result
        );
...
fn test_folded_scalar_explicit_indent_2space() {
    ...
    // at indent levels 1-5 with 2-space base indentation (Level 1).
    //
    ...
    // across the specified indent range.
    //
    // Bead: bf-1ana1
    // Acceptance Criteria:
    ...
    // Test continuation lines for each indent level with 2-space base indentation
    let continuation_lines = vec![
        // ===== Level 1 continuation lines (2-space per level) =====
        // Indent level 1: content should start at column 2 (0-indexed: 2 spaces)
        ...
        // ===== Level 2 continuation lines (4-space per level) =====
        // Indent level 2: content should start at column 4 (0-indexed: 4 spaces)
        ...
        // ===== Level 3 continuation lines (6-space per level) =====
        // Indent level 3: content should start at column 6 (0-indexed: 6 spaces)
        ...
        // ===== Level 4 continuation lines (8-space per level) =====
        // Indent level 4: content should start at column 8 (0-indexed: 8 spaces)
        ...
        // ===== Level 5 continuation lines (10-space per level) =====
        // Indent level 5: content should start at column 10 (0-indexed: 10 spaces)
...
fn test_folded_scalar_explicit_indent_4space() {
    // Test folded scalars with all three explicit indent modifiers (>, >-, >+)
    // at indent levels 1-5 with 4-space base indentation (Level 2).
    //
    // This test provides comprehensive coverage of folded scalar behavior with
    // explicit indent specification, ensuring all modifier variants work correctly
    // across the specified indent range at 4-space indentation level.
    //
    // Bead: bf-34f8s
    // Acceptance Criteria:
    // - Cover all three modifiers: > (plain), >- (strip), >+ (keep)
    // - Cover indent levels 1-5
    // - Verify folded scalar behavior with 4-space indentation
    // - Follow Section 12B.3 explicit indent infrastructure pattern
    ...
    for (line, _, _) in &test_cases {
        ...
        // Should not have 2 leading spaces (that would be level 1)
        assert!(
            !line.starts_with("  ") || line.starts_with("    "),
            "4-space test case should not start with only 2 spaces: '{}'",
            line
        );
    }
    ...
    // Test continuation lines for each indent level with 4-space base indentation
    let continuation_lines = vec![
        // ===== Level 1 continuation lines (2-space per level) =====
        // Indent level 1: content should start at column 2 (0-indexed: 2 spaces)
        ("      Content at indent level 1", vec![LineType::MappingKey, LineType::Unknown]),
