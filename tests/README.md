...
| `tests/aws-cli-compatibility/` | Real `aws` CLI, `rclone`, boto3, DuckDB/httpfs, litestream, and barman-cloud legs, plus `aws-sdk-go-v2` transfer managers and `TestHarness_GET_*` direct-GET paths against the real request pipeline | Full: `go test ./tests/aws-cli-compatibility/` (`make compat`). Without external clients: `go test -short ./tests/aws-cli-compatibility/` runs the in-process SDK/harness tests and skips client legs. `ARMOR_COMPAT_ENDPOINT` makes missing clients fatal and targets a live server — see [TEST_AGAINST_RUNNING_SERVER.md](aws-cli-compatibility/TEST_AGAINST_RUNNING_SERVER.md) | Full client gate needs `aws`, `rclone`, `boto3`, `duckdb`, `litestream`, `barman-cloud-backup`, `barman-cloud-restore`, and a PostgreSQL toolchain on `PATH`; fixtures render only ephemeral test credentials at runtime | Yes — `armor-build` installs the pinned clients and runs every client leg against the freshly built image |
| `tests/test_s3_basic_operations.py` | Real boto3/botocore SigV4 bucket, object, metadata, range, listing, and managed multipart operations against a running ARMOR image | `make compat-boto3` or `python3 tests/test_s3_basic_operations.py` with `ARMOR_COMPAT_ENDPOINT`, `ARMOR_COMPAT_ACCESS_KEY`, `ARMOR_COMPAT_SECRET_KEY`, and `ARMOR_BUCKET` set; `make compat` includes it in endpoint mode | Python 3 + boto3; the endpoint must be the built ARMOR image under test | The smaller boto3 leg in `tests/aws-cli-compatibility/` runs in the standing client matrix; this broader managed-multipart suite remains an additional image check |
...
| `tests/__init__.py` | Package marker so pytest imports the three script-test files under stable names | — | — | — |
