...
// Actions set is indexed by (see config.ACLEntry.Actions and config.validActions
// in internal/config/config.go). A classifier that returns one of these can be
...
// as an index into a config.ACLEntry.Actions set. An unrecognized HTTP method
