...
func TestInvalidCredentialRejection(t *testing.T) {
	...
	t.Run("Malformed signatures return 403 Forbidden", func(t *testing.T) {
		...
		req := createInvalidCredTestRequest(t, "GET", "/test-bucket/test-key", "", "TESTACCESSKEY", "REMOVED-NOT-A-SECRET-VALUE", nil)
	...
	t.Run("Expired request returns 403 Forbidden", func(t *testing.T) {
		...
		req := createSignedRequestWithTime(t, "GET", "/test-bucket/test-key", "", "TESTACCESSKEY", "REMOVED-NOT-A-SECRET-VALUE", &oldTime)
