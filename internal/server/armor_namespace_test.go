...
func TestArmorNamespaceProtectionWithAuth(t *testing.T) {
	...
	req := createInvalidCredTestRequest(t, "GET", "/test-bucket/.armor/protected/file", "", "TESTACCESSKEY", "REMOVED-NOT-A-SECRET-VALUE", nil)
	w := httptest.NewRecorder()
...
func TestNonArmorNamespaceAllowed(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req := createInvalidCredTestRequest(t, "GET", tc.url, "", "TESTACCESSKEY", "REMOVED-NOT-A-SECRET-VALUE", nil)
			w := httptest.NewRecorder()
