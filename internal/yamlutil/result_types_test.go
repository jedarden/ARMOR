...
func TestValidationResult_IsValid(t *testing.T) {
	...
	invalidResult := ValidationResult{
		...
		Valid:    false,
		Errors: []ValidationError{
			*NewValidationError("test.yaml", "required field missing", "server.name", "", ErrCodeRequiredField, 5, 0, ErrorTypeValidation, "", "server.name", ""),
		},
		Warnings: []ValidationError{},
	}
...
func TestValidationResult_IsValid_Consistency(t *testing.T) {
	...
	invalidWithErrors := ValidationResult{
		...
		Valid:    false,
		Errors: []ValidationError{
			*NewValidationError("test.yaml", "validation error", "", "", ErrCodeValidationFailed, 0, 0, ErrorTypeValidation, "", "", ""),
		},
	}
...
func TestNewResultMethods_Comprehensive(t *testing.T) {
	...
	t.Run("ValidationResult methods", func(t *testing.T) {
		...
		invalidResult := ValidationResult{
			...
			Valid:    false,
			Errors: []ValidationError{
				*NewValidationError("config.yaml", "port out of range", "server.port", "1-65535", ErrCodeInvalidValue, 15, 0, ErrorTypeValidation, "", "server.port", ""),
			},
		}
